                    ...
                    <div class="trow" v-for="ITEM in ITEMS" :key="ITEM.infoitems_id">
                        <template v-for="(col,index) in tabledef" :key="index">
                            <div class="tcol"   :style="{flex:col.flex}" :class="{'check-box': col.type=='checkbox',[index]:true}"  @click="selectrow(ITEM.infoitems_id,index ,ITEMS[0].InvoiceID)"  >
                                ...
                                <tag v-else-if="col.type=='tag'" :name="converySlotPostes.includes(ITEM.nextpost) ? ITEM[index] + ' - ' + ITEM.conveyorSlot : ITEM[index]" :style="converySlotPostes.includes(ITEM.nextpost) ? {backgroundColor:'transparent',border:'1px solid #000000',color:'#000000', fontSize: '10px', lineHeight: '14px', height: 'fit-content'} : {backgroundColor:'transparent',border:'1px solid #000000',color:'#000000'}"></tag>
