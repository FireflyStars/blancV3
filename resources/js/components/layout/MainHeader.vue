<template>
    <div class="row main-logo">
        <div class="col-12 p-0 d-flex align-items-center justify-content-between">
            <svg width="37" class="logo" height="37" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg" @click="slideinMenu">
...
<script>
    ...
    import {SIDEBAR_MODULE, SIDEBAR_SET_SLIDEIN, TOASTER_MESSAGE, TOASTER_MODULE, NEWORDER_MODULE, NEWORDER_PRELOAD_ORDER_CUSTOMER_SET} from "../../store/types/types";
    import SearchCustomer from '../miscellaneous/SearchCustomer'
    import { useRoute, useRouter} from 'vue-router'
    import ScanBarcode from '../miscellaneous/ScanBarcode'
    export default {
        name: "MainHeader",
        components: {SearchCustomer,ScanBarcode},
        setup(props,context){
            const store=useStore();
            const route =useRoute();
            const router=useRouter();
            const ShowscanBarcode=ref(false);
            const Scan= ref('');
            ...
            watch(() => Scan.value, (current_val, previous_val) => {
              if(Scan.value == false) {
                ShowscanBarcode.value= false;
              }else {
                ShowscanBarcode.value= true;
              }
