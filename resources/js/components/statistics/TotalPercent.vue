<template>
    <div class="d-flex align-items-center">
        <div class="fw-bold text-nowrap text-center mx-2" :class="'font-'+fontSize">{{ amount }} {{ symbol }}</div> 
        <div class="mx-2 d-flex align-items-center" v-if="arrow == 1">
        ...
        <div class="mx-2 d-flex align-items-center" v-else-if="arrow == -1">
            <svg width="13" :height="fontSize" viewBox="0 0 13 17" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
    ...
    setup(props){
        ...
        const percent = ref('');
        const arrow = ref(0);
        watch(() => props.amount, (cur_val, pre_val)=>{
            amount.value = cur_val;
            calcPercent();
        })
        watch(()=>props.pastAmount, (cur_val, pre_val)=>{
            pastAmount.value = cur_val;
            calcPercent();
        })
        const calcPercent = ()=>{
            if(amount.value > pastAmount.value && pastAmount.value != 0){
                percent.value = ((amount.value/pastAmount.value-1)*100).toFixed(0)+"%";
            }
            else if(amount.value == pastAmount.value){
                percent.value = '0%'
                arrow.value = 0
            }
            else if(amount.value < pastAmount.value){
                percent.value = (-1)*((amount.value/pastAmount.value-1)*100).toFixed(0)+"%";
                arrow.value = -1;
            }
            else if(pastAmount.value == 0){
                percent.value = '--'
                arrow.value = 0;
            }
        }
