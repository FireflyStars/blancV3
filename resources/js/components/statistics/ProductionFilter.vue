    ...
    <div class="d-flex filter">
        <div class="width-fit-content">
            <button class="btn rounded-2 bg-white body_small_bold py-2 d-flex align-items-center" :class="{active:showfilter,hasfilters:hasActiveFilters}" @click="toggleShow">
                ...
                {{customFilter ? 'Custom' : selectedValue}}
            </button>
        </div>
        <div class="d-flex align-items-center custom-filter-text" v-if="customFilter">
            <p :class="{ 'ps-3 m-0': customFilterText != ''}">{{customFilterText}}</p>
        </div>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                <div v-if="compareCustomFilter">
                    <div class="row mt-4">
                        <div class="col-3">
                            <date-picker v-model="compareStartDate" @update:modelValue="newValue => compareStartDate = newValue" name="pre_start_date" :droppos="{top:'auto',right:'auto',bottom:'auto',left:'0',transformOrigin:'top right'}" label="Début" :disabled-from-date="startDisabledtodate"></date-picker>
                        </div>
                    </div>
                    <div class="row mt-4">
                        <div class="col-3">
                            <date-picker v-model="compareEndDate" @update:modelValue="newValue => compareEndDate = newValue" name="pre_end_date" :droppos="{top:'auto',right:'auto',bottom:'auto',left:'0',transformOrigin:'top right'}" label="Fin" :disabled-from-date="endDisabledtodate"></date-picker>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        components:{SelectOptions,DatePicker,SwitchBtn},
         setup(props,context){
            let monthNames =["Jan","Fev","Mar","Apr",
                      "May","Jun","Jul","Aug",
                      "Sep", "Oct","Nov","Dec"];
            ...
            function applyFilter() {
                ...
                customFilterText.value=`compare to ${new Date(startDate.value).getDate()} ${monthNames[new Date(startDate.value).getMonth()]}, ${new Date(startDate.value).getFullYear()} - ${new Date(endDate.value).getDate()} ${monthNames[new Date(endDate.value).getMonth()]}, ${new Date(endDate.value).getFullYear()}`;
